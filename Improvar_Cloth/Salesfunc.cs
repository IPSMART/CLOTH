...
namespace Improvar
{
    public class Salesfunc : MasterHelpFa
    {
        ...
        public DataTable GetStock(string tdt, string gocd = "", string barno = "", string itcd = "", string mtrljobcd = "'FS'", string skipautono = "", string itgrpcd = "", string stylelike = "", string prccd = "WP", string taxgrpcd = "C001", string stktype = "", string brandcd = "", bool pendpslipconsider = true, bool shownilstock = false, string curschema = "", string finschema = "", bool mergeitem = false, bool mergeloca = false, bool exactbarno = true, string partcd = "", bool showallitems = false, string doctag = "", string SLCD = "", bool IncludeBaleStock = false, bool ShowOnlyFavitem = false, bool Phystk = false, bool skipNegetivStock = false, string fdt = "", bool exactstyleno = false, string loccd = "", string slcdfrrt = "", bool ShowShade = false)
        {
            ...
            if (Phystk == true)
            {
                ...
                else
                {
                    ...
                    sql += "select distinct a.barno, b.docdt, a.scmdiscrate, a.scmdisctype, " + Environment.NewLine;
                    sql += "row_number() over(partition by a.barno order by b.docdt) as rn " + Environment.NewLine;
                    sql += "from " + scm + ".t_txndtl a, " + scm + ".t_txn b " + Environment.NewLine;
                    sql += "where a.autono = b.autono and b.doctag = '" + doctag + "') where rn = 1 ) x, " + Environment.NewLine;
                }
                ...
                if (stylelike.retStr() != "")
                {
                    ...
                    {
                        sql += " (replace(d.styleno, '''', '') like '%" + stylelike.Replace("'", "") + "%') and " + Environment.NewLine;
            ...
            else
            {
                ...
                else
                {
                    ...
                    sql += "( " + Environment.NewLine;
                    sql += "select a.gocd, a.mtrljobcd, a.stktype, a.barno, a.itcd, a.partcd, a.colrcd, a.sizecd, '' shade, 0 cutlength, 0 dia, " + Environment.NewLine;
                    //if (MSYSCNFG.STKINCLPINV == "Y")
                    //{
                    //    sql += "sum(case a.stkdrcr when 'D' then a.qnty when 'C' then a.qnty*-1 when '0' then a.qnty*-1 end) balqnty, " + Environment.NewLine;
                    //    sql += "sum(case a.stkdrcr when 'D' then a.nos when 'C' then a.nos*-1 when '0' then a.qnty*-1 end) balnos " + Environment.NewLine;
                    ...
                    if (mtrljobcd.retStr() != "") sql += "a.mtrljobcd in (" + mtrljobcd + ") and " + Environment.NewLine;
                    sql += "c.docdt <= to_date('" + tdt + "','dd/mm/yyyy') " + Environment.NewLine;
                    sql += "and a.autono=g.autono(+) and a.slno = g.slno(+) " + Environment.NewLine;
                    if (IncludeBaleStock == false) sql += "and g.autono is null " + Environment.NewLine;
                    ...
                    if (pendpslipconsider == true)
                    {
                        ...
                        sql += "select a.gocd, a.mtrljobcd, a.stktype, a.barno, a.itcd, a.partcd, a.colrcd, a.sizecd, '' shade, 0 cutlength, 0 dia, " + Environment.NewLine;
                        sql += "sum(a.qnty*-1) balqnty, sum(a.nos*-1) balnos " + Environment.NewLine;
                        sql += "from " + scm + ".t_txndtl a, " + scm + ".t_batchmst b, " + scm + ".t_cntrl_hdr c, " + Environment.NewLine;
                        sql += "" + scm + ".m_doctype d, " + scm + ".t_txn_linkno e, " + scm + ".t_bale g " + Environment.NewLine;
                        sql += "where a.barno=b.barno(+) and a.autono=c.autono(+) and " + Environment.NewLine;
                        sql += "c.doccd=d.doccd(+) and d.doctype in ('SPSLP') and a.autono=e.linkautono(+) and e.autono is null and " + Environment.NewLine;
                        sql += "c.compcd='" + COM + "'";
                ...
                if (stylelike.retStr() != "")
                {
                    ...
                    else if (exactstyleno == true)
                    {
                        sql += " (replace(d.styleno, '''', '')=" + stylelike.ToUpper() + ") and " + Environment.NewLine;
                    }
                    else
                    {
                        sql += " (replace(d.styleno, '''', '') like '%" + stylelike.Replace("'", "") + "%') and " + Environment.NewLine;
        ...
        //    sql += "select '' gocd, '' mtrljobcd, '' stktype, a.barno, b.itcd, '' partcd, a.colrcd, a.sizecd, '' shade, 0 cutlength, 0 dia, 0 balqnty, 0 balnos " + Environment.NewLine;
        //    sql += "from " + scm + ".t_batchmst a, " + scm + ".m_sitem b, " + scm + ".m_cntrl_hdr c, " + scm + ".m_cntrl_loca d " + Environment.NewLine;
        //    sql += "where a.itcd=b.itcd(+) and b.m_autono=c.m_autono(+) and b.m_autono=d.m_autono(+) and " + Environment.NewLine;
        //    sql += "nvl(c.inactive_tag,'N')='N' and " + Environment.NewLine;
        //    if (barno.retStr() != "") sql += "upper(a.barno) in (" + barno + ") and " + Environment.NewLine;
        //    if (itcd.retStr() != "") sql += "a.itcd in (" + itcd + ") and " + Environment.NewLine;
        //    sql += "(d.compcd = '" + COM + "' or d.compcd is null) and (d.loccd='" + LOC + "' or d.loccd is null) " + Environment.NewLine;
        //    sql += ") a, " + Environment.NewLine;
        ...
        //            sql += " (replace(d.styleno,'''','')  like '%" + stylelike.Replace("'", "") + "%' or replace(d.styleno,'''','') ||' '||d.itnm like '%" + stylelike.Replace("'", "") + "%') and " + Environment.NewLine;
        //        }
        //    }
        ...
        //    if (itgrpcd.retStr() != "") sql += "d.itgrpcd in (" + itgrpcd + ") and " + Environment.NewLine;
        //    if (brandcd.retStr() != "") sql += "d.brandcd in (" + brandcd + ") and " + Environment.NewLine;
        //    if (partcd.retStr() != "") sql += "a.partcd='" + partcd + "' and " + Environment.NewLine;
        ...
        public DataTable GetBarHelp(string tdt, string gocd = "", string barno = "", string itcd = "", string mtrljobcd = "'FS'", string skipautono = "", string itgrpcd = "", string stylelike = "", string prccd = "WP", string taxgrpcd = "C001", string stktype = "", string brandcd = "", bool pendpslipconsider = true, bool shownilstock = false, string menupara = "", string curschema = "", string finschema = "", bool mergeitem = false, bool mergeloca = false, bool exactbarno = true, string partcd = "", bool showonlycommonbar = true, bool exactstyleno = false, string slcdfrrt = "")
        {
            ...
            if (mtrljobcd.retStr() != "") sql += "and a.mtrljobcd in (" + mtrljobcd + ")  " + Environment.NewLine;
            //sql += "and c.docdt <= to_date('" + tdt + "','dd/mm/yyyy') " + Environment.NewLine;
            sql += "union " + Environment.NewLine;
            //}
            //sql += "select '' gocd, '' mtrljobcd, '' stktype, a.barno, b.itcd, '' partcd, a.colrcd, a.sizecd, '' shade, 0 cutlength, 0 dia, 0 balqnty, 0 balnos ";
            //sql += "from " + scm + ".T_BATCHmst a, " + scm + ".m_sitem b, " + scm + ".m_cntrl_hdr c, " + scm + ".m_cntrl_loca d ";
            ...
            sql += "select distinct '' gocd, 'FS' mtrljobcd, '' stktype, a.barno, b.itcd, '' partcd, a.colrcd, a.sizecd, '' shade, 0 cutlength, 0 dia, 0 balqnty, 0 balnos " + Environment.NewLine;
            sql += "from " + scm + ".t_batchmst a, " + scm + ".m_sitem b, " + scm + ".m_cntrl_hdr c, " + scm + ".m_cntrl_loca d, " + scm + ".t_batchdtl e " + Environment.NewLine;
            sql += "where a.itcd=b.itcd(+) and b.m_autono=c.m_autono(+) and b.m_autono=d.m_autono(+) and a.barno=e.barno(+) and e.barno is null and " + Environment.NewLine;
            sql += "nvl(c.inactive_tag,'N')='N' and " + Environment.NewLine;
            if (barno.retStr() != "") sql += "upper(a.barno) in (" + barno + ") and " + Environment.NewLine;
            if (itcd.retStr() != "") sql += "a.itcd in (" + itcd + ") and " + Environment.NewLine;
            sql += "(d.compcd = '" + COM + "' or d.compcd is null) and (d.loccd='" + LOC + "' or d.loccd is null) " + Environment.NewLine;
        ...
        public string GetPendOrderSql(string slcd = "", string ordupto = "", string ordautono = "", string orderslno = "", string txnupto = "", string skipautono = "", string menupara = "SB", string brandcd = "", bool OnlyBal = true, string ordfromdt = "", string itcd = "", string agslcd = "", string slmslcd = "", string itgrpcd = "", string curschema = "", string finschema = "", bool Skipngtv = false)
        {
            ...
            sql += "nvl(a.qnty,0) - nvl(b.qnty,0) -nvl(c.qnty,0) balqnty,m.pdesign,m.partcd,p.partnm ,p.prtbarcode,f.clrbarcode,e.szbarcode,nvl(d.hsncode,h.hsncode)hsncode,nvl(d.negstock,h.negstock)negstock,n.PREFNO,n.PREFDT,nvl(n.PREFDT,o.docdt) prefdocdt,nvl(o.cancel, 'N')cancel from ";
        ...
        public DataTable GenStocktblwithVal(string calctype = "FIFO", string tdt = "", string barno = "", string mtrljobcd = "", string itgrpcd = "", string selitcd = "", string gocd = "", bool skipStkTrnf = true, string skipautono = "", bool summary = false, string unselitcd = "", string curschema = "", string LOCCD = "", string finschema = "", bool negstkrtfrmmaster = false, bool skipNegetivStock = false, string indatadocdt = "", bool ShowShade = false)
        {
            ...
            else
            {
                ...
                if (itgrpcd.retStr() != "") sqlc += "d.itgrpcd in (" + itgrpcd + ") and " + Environment.NewLine;
                if (barno.retStr() != "") sql += "a.barno in (" + barno + ") and " + Environment.NewLine;
                sqlc += "a.itcd=d.itcd(+) and d.itgrpcd=e.itgrpcd(+) " + Environment.NewLine;

                sql = "";
                sql += "select  a.autono,A.slno TXNSLNO,a.slno, c.doctag,a.stktype, conslcd, c.slcd, g.slnm, b.doccd, b.docdt, b.docno, substr(nvl(c.prefno,b.docno),16) blno, nvl(c.prefdt,b.docdt) bldt, " + Environment.NewLine; //a.slno, a.autono||a.slno autoslno
                ...
                sql += "a.stkdrcr in ('C') " + Environment.NewLine;
                if (tdt.retStr() != "") sql += "and b.docdt <= to_date('" + tdt + "','dd/mm/yyyy') " + Environment.NewLine;
                sql += "group by a.mtrljobcd, a.barno, a.itcd, a.mtrljobcd||a.itcd||a.barno " + Environment.NewLine;
                sql += sqldgrp;
                sql += "order by itcd " + Environment.NewLine;
