...
namespace Improvar.Controllers
{
    public class T_PhyStockController : Controller
    {
        ...
        public ActionResult SearchPannelData(TransactionPhyStockEntry VE, string SRC_SLCD, string SRC_DOCNO, string SRC_FDT, string SRC_TDT)
        {
            ...
            sql = "select a.autono, b.docno, to_char(b.docdt,'dd/mm/yyyy') docdt, b.doccd,a.gocd,c.gonm,sum(nvl(d.qnty,0))qnty,b.usr_id,a.trem ";
            ...
            sql += "group by a.autono, b.docno, to_char(b.docdt,'dd/mm/yyyy'), b.doccd,a.gocd,c.gonm,b.usr_id,a.trem ";
            sql += "order by docdt, docno ";
            DataTable tbl = Master_Help.SQLquery(sql);

            System.Text.StringBuilder SB = new System.Text.StringBuilder();
            var hdr = "Document Number" + Cn.GCS() + "Document Date" + Cn.GCS() + "Godown Name" + Cn.GCS() + "Quantity" + Cn.GCS() + "Remarks" + Cn.GCS() + "Created By" + Cn.GCS() + "AUTO NO";
            for (int j = 0; j <= tbl.Rows.Count - 1; j++)
            {
                SB.Append("<tr><td><b>" + tbl.Rows[j]["docno"] + "</b> [" + tbl.Rows[j]["doccd"] + "]" + " </td><td>" + tbl.Rows[j]["docdt"] + " </td><td><b>" + tbl.Rows[j]["gonm"] + "</b> [" + tbl.Rows[j]["gocd"] + "]</td><td>" + tbl.Rows[j]["qnty"] + " </td><td>" + tbl.Rows[j]["trem"] + " </td><td>" + tbl.Rows[j]["usr_id"] + " </td><td>" + tbl.Rows[j]["autono"] + " </td></tr>");
            }
            return PartialView("_SearchPannel2", Master_Help.Generate_SearchPannel(hdr, SB.ToString(), "6", "6"));
        }
                    ...
                    if (VE.DefaultAction == "A" || VE.DefaultAction == "E")
                    {
                        ...
                        for (int i = 0; i <= VE.TPHYSTK.Count - 1; i++)
                        {
                            if (VE.TPHYSTK[i].SLNO != 0 && VE.TPHYSTK[i].QNTY != 0)
                            {
                                ...
                                TPHYSTK.EMD_NO = TBHDR.EMD_NO;
                                TPHYSTK.DTAG = TBHDR.DTAG;
