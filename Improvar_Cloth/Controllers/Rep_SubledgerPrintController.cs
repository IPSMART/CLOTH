            ...
            try
            {
                ...
                if (reptype == "All")
                {
                    ...
                    if (tbl.Rows.Count != 0)
                    {
                        ...
                        if (VE.Checkbox3 != true) HC.GetPrintHeader(IR, "PARTYCD", "string", "c,12", "Party Group Code");
                        if (VE.Checkbox3 != true) HC.GetPrintHeader(IR, "PARTYNM", "string", "c,26", "Party Group Name");
                        ...
                        if (VE.Checkbox3 != true) HC.GetPrintHeader(IR, "GSTNO", "string", "c,17", "GSTNO");
                        ...
                        if (VE.Checkbox3 != true)HC.GetPrintHeader(IR, "TCSAPPL", "string", "c,5", "TCS Applicable");
                        if (VE.Checkbox3 != true) HC.GetPrintHeader(IR, "TOT194Q", "string", "c,5", "Turnover Limit Cross For 194Q");
                        ...
                        while (i <= maxR)
                        {
                            ...
                            while (tbl.Rows[i][fld].retStr() == fldval)
                            {
                                ...
                                if (VE.Checkbox3 != true) IR.Rows[rNo]["GSTNO"] = tbl.Rows[i]["GSTNO"];
                                if (VE.Checkbox3 != true)IR.Rows[rNo]["ADHAARNO"] = tbl.Rows[i]["ADHAARNO"];
                                ...
                                if (VE.Checkbox3 != true)   IR.Rows[rNo]["USR_ENTDT"] = tbl.Rows[i]["USR_ENTDT"];
                                if (VE.Checkbox3 != true) IR.Rows[rNo]["AGSLCD"] = tbl.Rows[i]["AGSLCD"];
                                IR.Rows[rNo]["AGSLNM"] = tbl.Rows[i]["AGSLNM"];
                                if (VE.Checkbox3 != true) IR.Rows[rNo]["TCSAPPL"] = tbl.Rows[i]["TCSAPPL"].retStr() == "Y" ? "Yes" : "No";
                                if (VE.Checkbox3 != true) IR.Rows[rNo]["TOT194Q"] = tbl.Rows[i]["TOT194Q"].retStr() == "Y" ? "Yes" : "No";
                                i = i + 1;
                                if (i > maxR) break;
                            }
