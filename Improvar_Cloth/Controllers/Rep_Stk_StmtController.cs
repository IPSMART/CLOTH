...
namespace Improvar.Controllers
{
    public class Rep_Stk_StmtController : Controller
    {
        ...
        public ActionResult Rep_Stk_Stmt(FormCollection FC, ReportViewinHtml VE)
        {
            try
            {
                ...
                if (summary == "D" || (summary == "F" && repon == "D"))
                {
                    ...
                    return Details(FC, VE, tbl, COM, LOC, asdt, prccd, qdsp, summary, rateon, fifotbl, pghdr2);
                }
                else if (summary == "S" || (summary == "F" && repon == "S"))
                {
                    //DataTable tbl = Salesfunc.GetStock(asdt, selgocd, "", selitcd, "FS".retSqlformat(), "", selitgrpcd, "", "CP");
                    return Summary(FC, VE, tbl, COM, LOC, asdt, prccd, qdsp, ignoreitems, summary, rateon, fifotbl, pghdr2);
                }
                else if (summary == "G")
                {
                    //DataTable tbl = Salesfunc.GetStock(asdt, selgocd, "", selitcd, "FS".retSqlformat(), "", selitgrpcd, "", "CP");
                    return Godownwise(FC, VE, tbl, COM, LOC, asdt, prccd, qdsp, pghdr2);
                }
                else if (summary == "B")
                {
                    return SummaryWise_Barcode(FC, VE, COM, LOC, asdt, prcd, selitgrpcd, selgocd, selitcd, pghdr2, mtrljobcd);
                }
                else if (summary == "P")
                {
                    if (repon == "D")
                    {
                        return PhysicalStock_Detail(FC, VE, tbl, COM, LOC, asdt, prccd, qdsp, summary, pghdr2);
                    }
                    else
                    {
                        return PhysicalStock_Summary(FC, VE, tbl, COM, LOC, asdt, prccd, qdsp, ignoreitems, summary, pghdr2);
        ...
        public ActionResult SummaryWise_Barcode(FormCollection FC, ReportViewinHtml VE, string COM, string LOC, string ASDT, string PRCCD, string ITGRPCD, string GOCD, string ITCD, string pghdr2, string mtrljobcd)
        {
            string errorrow = "";
            try
            {
                ...
                if (mtrljobcd.retStr() != "") query += "and a.mtrljobcd in (" + mtrljobcd + ") ";
                ...
                while (i <= maxR)
                {
                    ...
                    netstktrans = (tbl1.Rows[i]["doctag"].retStr() == "ST" || tbl1.Rows[i]["doctag"].retStr() == "TO" || tbl1.Rows[i]["doctag"].retStr() == "TI") ? tbl1.Rows[i]["qnty"].retDbl() : 0;

                    netadj = (tbl1.Rows[i]["doctag"].retStr() == "SC" || tbl1.Rows[i]["doctag"].retStr() == "CN" || tbl1.Rows[i]["doctag"].retStr() == "AD" || tbl1.Rows[i]["doctag"].retStr() == "WA") || (tbl1.Rows[i]["doctag"].retStr() == "SA") || (tbl1.Rows[i]["doctag"].retStr() == "AD") ? tbl1.Rows[i]["qnty"].retDbl() : 0;
                    ...
                    if (summarybarcode != null && summarybarcode.Rows.Count > 0)
                    {
                        ...
                        existdr = (from DataRow dr in summarybarcode.Rows where dr["key"].retStr() == keyval select dr).FirstOrDefault();
                    }
            ...
            {
                Cn.SaveException(ex, "");
                return Content(ex.Message + errorrow);
            }
        }
        public ActionResult PhysicalStock_Detail(FormCollection FC, ReportViewinHtml VE, DataTable tbl, string COM, string LOC, string ASDT, string PRCCD, string QDSP, string summary, string pghdr2)
        {
            Models.PrintViewer PV = new Models.PrintViewer();
            HtmlConverter HC = new HtmlConverter();
            DataTable IR = new DataTable("");
        ...
        public ActionResult PhysicalStock_Summary(FormCollection FC, ReportViewinHtml VE, DataTable tbl, string COM, string LOC, string ASDT, string PRCCD, string QDSP, bool ignoreitems, string summary, string pghdr2)
        {
            Models.PrintViewer PV = new Models.PrintViewer();
            HtmlConverter HC = new HtmlConverter();
            DataTable IR = new DataTable("");
