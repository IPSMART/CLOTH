...
namespace Improvar.Controllers
{
    public class M_SubLeg_SDdtlController : Controller
    {
        ...
        public ActionResult SearchPannelData(string SRC_FLAG)
        {
            ...
            var hdr = "Sub Ledger Code" + Cn.GCS() + "Sub Ledger Name" + Cn.GCS() + "GST" + Cn.GCS() + "cashdisc%" + Cn.GCS() + "District" + Cn.GCS() + "Price list Code" + Cn.GCS() + "Company Code" + Cn.GCS() + "Agent Name" + Cn.GCS() + "Agent Code" ;
            for (int j = 0; j <= MDT.Rows.Count - 1; j++)
            {
                SB.Append("<tr><td>" + MDT.Rows[j]["SLCD"].retStr() + "</td><td>" + MDT.Rows[j]["SLNM"].retStr() + " </td><td>" + MDT.Rows[j]["GSTNO"].retStr() + " </td><td> " + MDT.Rows[j]["CASHDISCPR"].retStr() + "</td><td> " + MDT.Rows[j]["DISTRICT"].retStr() + "</td><td>" + MDT.Rows[j]["PRCCD"].retStr() + " </td><td> " + MDT.Rows[j]["COMPCD"].retStr() + "</td><td> " + MDT.Rows[j]["AGSLNM"].retStr() + "</td><td> " + MDT.Rows[j]["AGSLCD"].retStr() + "</td></tr>");
            }
            return PartialView("_SearchPannel2", masterHelpFa.Generate_SearchPannel(hdr, SB.ToString(), "0" + Cn.GCS() + "6", "6"));
            ...
            using (var transaction = DB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (VE.DefaultAction == "A" || VE.DefaultAction == "E")
                    {
                        ...
                        MSUBLEGCOM.CASHDISCPR = VE.M_SUBLEG_COM.CASHDISCPR;
                        //grid saving
