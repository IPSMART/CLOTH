    ...
    public class T_OUTRECPROCESSController : Controller
    {
            ...
            try
            {
                ...
                else
                {
                    ...
                    switch (VE.MENU_PARA)
                    {
                        ...
                        case "IR":
                            ViewBag.formname = "Receive for Iron"; break;
                        case "IRU":
                            ViewBag.formname = "Return from Iron w/o Work"; break;
        ...
        public TransactionOutRecProcess Navigation(TransactionOutRecProcess VE, ImprovarDB DB, int index, string searchValue)
        {
            ...
            if (VE.IndexKey.Count != 0)
            {
                ...
                str += "b.ITCD,d.FABITCD,d.STYLENO,d.UOMcd,e.COLRNM,b.COLRCD,b.SIZECD,b.SHADE,a.NOS,a.QNTY,b.ITREMARK,f.ITNM FABITNM,b.sample,g.COMMONUNIQBAR,a.SHORTQNTY,d.styleno||' '||d.itnm itstyle,a.JOBTXNTY,b.mtrljobcd,B.ORDAUTONO ";
        ...
        public ActionResult SearchPannelData(TransactionOutRecProcess VE, string SRC_SLCD, string SRC_DOCNO, string SRC_FDT, string SRC_TDT)
        {
            ...
            sql = "select a.autono, b.docno, to_char(b.docdt,'dd/mm/yyyy') docdt, b.doccd, a.slcd, c.slnm, c.district,c.gstno,a.jobcd," + Environment.NewLine;
            sql += "d.jobnm, nvl(a.blamt,0) blamt,a.PREFNO,SUM(AMT) AS t_amt,a.PREFDT, nvl(b.cancel,'N')cancel,b.docdt tchdocdt " + Environment.NewLine;
            sql += "from " + scm + ".t_txn a, " + scm + ".t_cntrl_hdr b, " + scmf + ".m_subleg c ," + scm + ".m_jobmst d," + scm + ".t_txndtl e " + Environment.NewLine;
            sql += "where a.autono=b.autono and e.autono=b.autono(+) and a.slcd=c.slcd(+) and a.jobcd=d.jobcd(+) and b.doccd in (" + doccd + ") and " + Environment.NewLine;
            if (SRC_FDT.retStr() != "") sql += "b.docdt >= to_date('" + SRC_FDT.retDateStr() + "','dd/mm/yyyy') and " + Environment.NewLine;
            if (SRC_TDT.retStr() != "") sql += "b.docdt <= to_date('" + SRC_TDT.retDateStr() + "','dd/mm/yyyy') and " + Environment.NewLine;
            if (SRC_DOCNO.retStr() != "") sql += "(b.vchrno like '%" + SRC_DOCNO.retStr() + "%' or b.docno like '%" + SRC_DOCNO.retStr() + "%') and " + Environment.NewLine;
            if (SRC_SLCD.retStr() != "") sql += "(a.slcd like '%" + SRC_SLCD.retStr() + "%' or upper(c.slnm) like '%" + SRC_SLCD.retStr().ToUpper() + "%') and " + Environment.NewLine;
            sql += "b.loccd='" + LOC + "' and b.compcd='" + COM + "' and b.yr_cd='" + yrcd + "' " + Environment.NewLine;
            sql += "group by a.autono, b.docno,b.docdt,b.doccd, a.slcd, c.slnm, c.district,c.gstno,a.jobcd,d.jobnm,blamt,a.PREFNO,a.PREFDT,cancel " + Environment.NewLine;
            ...
            for (int j = 0; j <= tbl.Rows.Count - 1; j++)
            {
                ...
                SB.Append("<tr><td><b>" + tbl.Rows[j]["docno"] + "</b> [" + tbl.Rows[j]["doccd"] + "]" + cancel + " </td><td>" + tbl.Rows[j]["docdt"] + " </td><td><b>"
                    ...
                    + tbl.Rows[j]["jobnm"] + "(" + tbl.Rows[j]["jobcd"] + ") </td><td>" + tbl.Rows[j]["PREFNO"] + " </td><td>"
                            + tbl.Rows[j]["PREFDT"].retStr().Remove(10) + " </td><td>" + tbl.Rows[j]["t_amt"] + " </td><td>" + tbl.Rows[j]["autono"] + " </td></tr>");
            }
            ...
            try
            {
                ...
                if (val.retStr() == "" || tbl.Rows.Count > 1)
                {
                    ...
                    var hdr = "Doc No." + Cn.GCS() + "Doc Dt." + Cn.GCS() + "Uniq. No." + Cn.GCS() + "Bar Code No" + Cn.GCS()
                        ...
                        + Cn.GCS() + "progautoslno" + Cn.GCS() + "Item Remarks" + Cn.GCS() + "Fabric Item" + Cn.GCS() + "Order No." + Cn.GCS() + "Order Slno." + Cn.GCS() + "Make StyleNo.";
        ...
        public ActionResult SAVE(FormCollection FC, TransactionOutRecProcess VE)
        {
            ...
            .Union(txndtldata.Where(a => !barcodedata.Any(a1 => a1.ITCD == a.ITCD && a1.QTY == a.QTY && a1.NOS == a.NOS))).ToList();
                        ...
                        for (int i = 0; i <= VE.TTXNDTL.Count - 1; i++)
                        {
                            if (VE.TTXNDTL[i].SLNO != 0 && VE.TTXNDTL[i].ITCD != null)
                            {
                                ...
                                dbsql = masterHelp.InsVch_GST(TTXN.AUTONO, TTXN.DOCCD, TTXN.DOCNO, TTXN.DOCDT.ToString(), TTXN.EMD_NO.Value, TTXN.DTAG, cr,
                                        Convert.ToSByte(isl), Convert.ToSByte(gs), TTXN.SLCD, strblno, strbldt, VE.TTXNDTL[i].HSNCODE, VE.TTXNDTL[i].ITSTYLE, (VE.TTXNDTL[i].BLQNTY == null || VE.TTXNDTL[i].BLQNTY == 0 ? VE.TTXNDTL[i].QNTY.retDbl() : VE.TTXNDTL[i].BLQNTY.retDbl()), (VE.TTXNDTL[i].UOM == null ? VE.TTXNDTL[i].UOM : VE.TTXNDTL[i].UOM),
                ...
                else if (VE.DefaultAction == "V")
                {
                    ...
                    //dbsql = masterHelp.T_Cntrl_Hdr_Updt_Ins(VE.T_TXN.AUTONO, "D", "F", null, null, null, VE.T_TXN.DOCDT.retStr(), null, null, null, null, null, null, null, 0, VE.Audit_REM);
                    //dbsql1 = dbsql.Split('~'); OraCmd.CommandText = dbsql1[0]; OraCmd.ExecuteNonQuery(); OraCmd.CommandText = dbsql1[1]; OraCmd.ExecuteNonQuery();
        ...
        //    }
        //}
        public ActionResult Print(TransactionOutRecProcess VE)
        {
            try
            {
                Cn.getQueryString(VE);
                ...
                repDoc.ActionName = "Rep_RecChallan_Print";
