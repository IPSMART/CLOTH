    ...
    public class Rep_Stk_LegController : Controller
    {
        ...
        public ActionResult Rep_Stk_Leg(FormCollection FC, ReportViewinHtml VE)
        {
            try
            {
                ...
                sql += "c.itnm,c.styleno||' '||c.itnm itstyle,c.uomcd, d.uomnm, a.rate,a.pageslno, a.nos, a.qnty, nvl(a.netamt,0) netamt,a.txblval, b.batchnos,e.tgonm,f.fgonm,g.baleno " + Environment.NewLine;
                sql += ",a.mtrljobcd,a.mtrljobnm" + (ShowShade == true ? ",b.shade" : "") + " " + Environment.NewLine;
                ...
                sql += ",a.mtrljobcd,k.mtrljobnm " + Environment.NewLine;
                sql += "from " + scm1 + ".t_txndtl a, " + scm1 + ".t_txn b, " + scm1 + ".t_cntrl_hdr c," + scm1 + ".m_doctype d, " + Environment.NewLine;
                sql += scmf + ".m_subleg i, " + scm1 + ".t_bale j, " + scm1 + ".m_mtrljobmst k " + Environment.NewLine;
                sql += "where a.autono=b.autono(+) and a.autono=c.autono(+) and c.doccd=d.doccd(+) and b.slcd=i.slcd(+) and a.mtrljobcd=k.mtrljobcd(+) and " + Environment.NewLine;
                ...
                sql += "group by a.autono, a.slno, a.autono||a.slno, a.stkdrcr, c.docno, c.docdt,d.docnm, b.prefno, b.prefdt, i.slnm, i.gstno,i.district, a.itcd, a.rate, a.txblval,a.pageslno " + Environment.NewLine;
                sql += ",a.mtrljobcd,k.mtrljobnm " + Environment.NewLine;
                sql += ") a, " + Environment.NewLine;
                if (BatchExist == true)
                {
                    sql += "( select a.autono, a.txnslno slno, a.autono||a.txnslno autoslno, listagg(b.batchno,',') within group (order by a.autono,a.slno) batchnos" + (ShowShade == true ? ",listagg(a.shade,',') within group (order by a.autono,a.slno) shade" : "") + " " + Environment.NewLine;
                    sql += "from " + scm1 + ".t_batchdtl a, " + scm1 + ".t_batchmst b where a.barno=b.barno(+) group by a.autono, a.txnslno, a.autono||a.txnslno ) b, " + Environment.NewLine;
                }
                else
                {
                ...
                sql += "order by ";
                sql += "mtrljobnm,mtrljobcd, ";
                sql += "itnm, itcd" + (ShowShade == true ? ",shade" : "") + ", docdt, stkdrcr desc, autono ";
                ...
                while (i <= maxR)
                {
                    ...
                    {
                        chkval = tbl.Rows[i]["itcd"].ToString() + (ShowShade == true ? tbl.Rows[i]["shade"].retStr().Trim() : "");
                        iop = 0; idr = 0; icr = 0; icls = 0; idramt = 0; icramt = 0;

                        IR.Rows.Add(""); rNo = IR.Rows.Count - 1;
                        IR.Rows[rNo]["Dammy"] = "<span style='font-weight:100;font-size:9px;'>" + " " + tbl.Rows[i]["itcd1"].ToString() + "  " + " </span>" + tbl.Rows[i]["itstyle"].ToString() + (ShowShade == true ? (tbl.Rows[i]["shade"].retStr().Trim() == "" ? "" : " (Shade : " + tbl.Rows[i]["shade"].retStr().Trim() + ")") : "");
                        IR.Rows[rNo]["Dammy"] = IR.Rows[rNo]["Dammy"] + " </span>" + " [" + tbl.Rows[i]["uomcd"] + "]";
                        IR.Rows[rNo]["flag"] = "font-weight:bold;font-size:13px;";
                        while (tbl.Rows[i]["mtrljobcd"].ToString() == mtrljob && tbl.Rows[i]["itcd"].ToString() + (ShowShade == true ? tbl.Rows[i]["shade"].retStr().Trim() : "") == chkval)
                        {
                            double bqnty = 0, bnos = 0, bval = 0, bamt = 0;
                            while (tbl.Rows[i]["mtrljobcd"].ToString() == mtrljob && tbl.Rows[i]["itcd"].ToString() + (ShowShade == true ? tbl.Rows[i]["shade"].retStr().Trim() : "") == chkval && Convert.ToDateTime(tbl.Rows[i]["docdt"]) < Convert.ToDateTime(fdt))
                            {
                            ...
                            while (tbl.Rows[i]["mtrljobcd"].ToString() == mtrljob && tbl.Rows[i]["itcd"].ToString() + (ShowShade == true ? tbl.Rows[i]["shade"].retStr().Trim() : "") == chkval && Convert.ToDateTime(tbl.Rows[i]["docdt"]) <= Convert.ToDateTime(tdt))
                            {
                                //if (rateqntybag == "B") dbqty = Convert.ToDouble(tbl.Rows[i]["nos"]);
                        ...
                        IR.Rows.Add(""); rNo = IR.Rows.Count - 1;
                        IR.Rows[rNo]["dammy"] = "";
                        IR.Rows[rNo]["slnm"] = "Totals " + tbl.Rows[i - 1]["mtrljobnm"] + " - (" + tbl.Rows[i - 1]["styleno"] + tbl.Rows[i - 1]["itnm"] + ")" + (ShowShade == true ? (tbl.Rows[i - 1]["shade"].retStr().Trim() == "" ? "" : " (Shade : " + tbl.Rows[i - 1]["shade"].retStr().Trim() + ")") : "");
