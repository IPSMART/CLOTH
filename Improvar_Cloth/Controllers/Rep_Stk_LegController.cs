    ...
    public class Rep_Stk_LegController : Controller
    {
        ...
        public ActionResult Rep_Stk_Leg()
        {
            try
            {
                ...
                else
                {
                    ...
                    VE.TEXTBOX2 = MasterHelp.ComboFill("fgocd", VE.DropDown_list2, 0, 1);
                    VE.TEXTBOX3 = MasterHelp.ComboFill("tgocd", VE.DropDown_list2, 0, 1);
        ...
        public ActionResult Rep_Stk_Leg(FormCollection FC, ReportViewinHtml VE)
        {
            try
            {
                ...
                string selitcd = "", plist = "", selgocd = "", selgonm = "", LOCCD = "", FGOCD = "", TGOCD = "";

                //if (FC.AllKeys.Contains("Godown"))
                //{
                //    selgocd = FC["Godown"].ToString().retSqlformat();
                //}
                if (FC.AllKeys.Contains("fgocdvalue")) FGOCD = CommFunc.retSqlformat(FC["fgocdvalue"].ToString());
                if (FC.AllKeys.Contains("tgocdvalue")) TGOCD = CommFunc.retSqlformat(FC["tgocdvalue"].ToString());
                ...
                //sql += "c.itnm,c.styleno||' '||c.itnm itstyle,c.uomcd, d.uomnm, a.rate, a.nos, a.qnty, nvl(a.netamt,0) netamt,a.txblval, b.batchnos,a.baleno,a.loccd,a.locnm from ";
                sql += "c.itnm,c.styleno||' '||c.itnm itstyle,c.uomcd, d.uomnm, a.rate, a.nos, a.qnty, nvl(a.netamt,0) netamt,a.txblval,a.baleno,a.gocd,a.gonm from ";
                ...
                sql += "group by a.autono, a.slno, a.autono||a.slno, a.stkdrcr, c.docno, c.docdt,d.docnm, b.prefno, b.prefdt, i.slnm, i.gstno,i.district, a.itcd, a.rate, a.txblval ";
                sql += ",a.baleno,b.gocd,j.gonm ";
                sql += " ) a, ";
                ...
                sql += "where a.itcd=c.itcd(+) and c.uomcd=d.uomcd(+) ";
                ...
                HC.GetPrintHeader(IR, "baleno", "string", "c,16", "Baleno");
                HC.GetPrintHeader(IR, "fgonm", "string", "c,16", "From Godown");
                HC.GetPrintHeader(IR, "tgonm", "string", "c,16", "To Godown");
                ...
                while (i <= maxR)
                {
                    ...
                    while (tbl.Rows[i]["itcd"].ToString() == chkval)
                    {
                        ...
                        while (tbl.Rows[i]["itcd"].ToString() == chkval && Convert.ToDateTime(tbl.Rows[i]["docdt"]) <= Convert.ToDateTime(tdt))
                        {
                            ...
                            IR.Rows[rNo]["baleno"] = tbl.Rows[i]["baleno"].ToString();
                            if (tbl.Rows[i]["stkdrcr"].ToString() == "D")
                            {
                                ...
                                IR.Rows[rNo]["tgonm"] = tbl.Rows[i]["gonm"].ToString();
                            }
                            else
                            {
                                ...
                                IR.Rows[rNo]["fgonm"] = tbl.Rows[i]["gonm"].ToString();
                            }
                            icls = idr - icr;
