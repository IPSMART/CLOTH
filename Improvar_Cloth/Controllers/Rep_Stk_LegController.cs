    ...
    public class Rep_Stk_LegController : Controller
    {
        ...
        public ActionResult Rep_Stk_Leg()
        {
            try
            {
                ...
                else
                {
                    ...
                    VE.DropDown_list2 = (from i in DB.M_GODOWN
        ...
        public ActionResult Rep_Stk_Leg(FormCollection FC, ReportViewinHtml VE)
        {
            try
            {
                ...
                //string sql = "";
                //sql += "select a.autono, a.slno, a.autoslno, a.stkdrcr, a.docno, a.docdt, a.pblno, a.pbldt, a.slnm, a.gstno, a.itcd||nvl(a.damstock,' ') itcd, a.damstock, ";
                //sql += "c.itnm, c.packsize, c.uomcd, d.uomnm, a.nos, a.qnty, nvl(a.netamt,0) netamt, b.batchnos from ";
                //sql +=  "(select a.autono, a.slno, a.autono||a.slno autoslno, a.stkdrcr, c.docno, c.docdt, b.pblno, b.pbldt, i.slnm, i.gstno, a.itcd, a.damstock, ";
                //sql += "sum(a.basamt - nvl(a.stddiscamt,0) - nvl(a.discamt,0) + nvl(a.othramt,0)) netamt, ";
                ...
                //sql +=  "a.stkdrcr in ('D','C') and  nvl(c.cancel,'N') = 'N' and b.itgrpcd='" + itgrpcd + "' and c.compcd='" + COM + "'  and ";
                ...
                //sql += "group by a.autono, a.slno, a.autono||a.slno, a.stkdrcr, c.docno, c.docdt, b.pblno, b.pbldt, i.slnm, i.gstno, a.itcd, a.damstock ) a, ";

                //sql += "( select a.autono, a.slno, a.autono||a.slno autoslno, listagg(b.batchno,',') within group (order by a.autono,a.slno) batchnos ";
                //sql += "from " + scm1 + ".t_batchdtl a, " + scm1 + ".t_batchmst b where a.batchautono=b.batchautono(+) group by a.autono, a.slno, a.autono||a.slno ) b, ";

                //sql += scm1 + ".m_sitem c, " + scmf + ".m_uom d ";
                //sql += "where a.autoslno=b.autoslno(+) and a.itcd=c.itcd(+) and c.uomcd=d.uomcd(+) ";
                //sql += "order by itnm, packsize, itcd, docdt, stkdrcr desc, autono ";
                string sql = "";
                sql += "select a.autono, a.slno, a.autoslno, a.stkdrcr, a.docno, a.docdt, a.prefno, a.prefdt, a.slnm, a.gstno, a.itcd||nvl(a.stktype,' ') itcd, a.stktype, ";
                sql += "c.itnm, c.uomcd, d.uomnm, a.nos, a.qnty, nvl(a.netamt,0) netamt, b.batchnos from ";
                sql += "(select a.autono, a.slno, a.autono||a.slno autoslno, a.stkdrcr, c.docno, c.docdt, b.prefno, b.prefdt, i.slnm, i.gstno, a.itcd, a.stktype, ";
                sql += "a.netamt, ";
                sql += "sum(nvl(a.nos,0)) nos, sum(nvl(a.qnty,0)) qnty ";
                sql += "from " + scm1 + ".t_txndtl a, " + scm1 + ".t_txn b, " + scm1 + ".t_cntrl_hdr c, ";
                sql += scmf + ".m_subleg i , " + scm1 + ".m_sitem j, " + scm1 + ".m_group k ";
                sql += "where a.autono=b.autono(+) and a.autono=c.autono(+) and b.slcd=i.slcd(+) and j.itgrpcd=k.itgrpcd(+) and ";
                sql += "a.stkdrcr in ('D','C') and  nvl(c.cancel,'N') = 'N' and j.itgrpcd='" + itgrpcd + "' and c.compcd='" + COM + "'  and ";
                if (selitcd.retStr() != "") sql += "a.itcd in (" + selitcd + ") and ";
                if (LOCCD != "") { sql += " c.loccd in(" + LOCCD + ") and "; } else { sql += " c.loccd='" + LOC + "' and "; }
                if (plist.retStr() != "")
                {
                ...
                if (selgocd.retStr() != "") sql += "b.gocd in (" + selgocd + ") and ";
                sql += "c.docdt <= to_date('" + tdt + "','dd/mm/yyyy') ";
                sql += "group by a.autono, a.slno, a.autono||a.slno, a.stkdrcr, c.docno, c.docdt, b.prefno, b.prefdt, i.slnm, i.gstno, a.itcd, a.stktype,a.netamt ) a, ";

                sql += "( select a.autono, a.slno, a.autono||a.slno autoslno, listagg(b.batchno,',') within group (order by a.autono,a.slno) batchnos ";
                sql += "from " + scm1 + ".t_batchdtl a, " + scm1 + ".t_batchmst b where a.barno=b.barno(+) group by a.autono, a.slno, a.autono||a.slno ) b, ";
                ...
                while (i <= maxR)
                {
                    ...
                    IR.Rows[rNo]["dammy"] = tbl.Rows[i]["itnm"].ToString() + (tbl.Rows[i]["stktype"].ToString() == "D"?" (Damage)":"") + tbl.Rows[i]["itcd"].ToString() + "] [" + tbl.Rows[i]["uomcd"].ToString() + "]";
                    ...
                    IR.Rows.Add(""); rNo = IR.Rows.Count - 1;
                    IR.Rows[rNo]["dammy"] = "";
                    IR.Rows[rNo]["slnm"] = "Totals (" + tbl.Rows[i - 1]["itnm"] + ")";
