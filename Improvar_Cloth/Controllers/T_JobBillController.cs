...
namespace Improvar.Controllers
{
    public class T_JobBillController : Controller
    {
        ...
        public ActionResult SearchPannelData()
        {
            ...
            string sql = "";
            sql += "select distinct a.autono, d.doccd, to_char(d.docdt,'dd/mm/yyyy') docdt, d.docno, d.doconlyno, c.jobnm,nvl(d.cancel,'N')cancel, ";
            sql += "a.slcd, e.slnm, a.pblno, d.docdt ddocdt, a.blamt ";
            ...
            var hdr = "Doc No" + Cn.GCS() + "Doc Date" + Cn.GCS() + "Job Name" + Cn.GCS() + "P/Blno" + Cn.GCS() + "Amount" + Cn.GCS() + "Party Name" + Cn.GCS() + "AUTONO";
            for (int j = 0; j <= tbl.Rows.Count - 1; j++)
            {
                ...
                SB.Append("<tr><td><b>" + tbl.Rows[j]["docno"] + "</b> [" + tbl.Rows[j]["doccd"] + "]" + cancel + "</td><td>" + tbl.Rows[j]["docdt"] + "</td><td>" + tbl.Rows[j]["jobnm"] + "</td><td>" + tbl.Rows[j]["pblno"] +
                    " </td><td>" + tbl.Rows[j]["blamt"] + "</td><td><b>" + tbl.Rows[j]["slnm"] + " </b> (" + tbl.Rows[j]["slcd"] + ") </td><td>" + tbl.Rows[j]["autono"] + " </td></tr>");
            }
            return PartialView("_SearchPannel2", masfa.Generate_SearchPannel(hdr, SB.ToString(), "6", "6"));
        }
